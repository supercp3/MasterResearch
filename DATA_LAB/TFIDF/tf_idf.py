...
class TFIDF:
	...
	def similar_string(self,a,b):
		...
		res=self.cosine_similarity(vec1,vec2)
		return res

	def compute_tfidf(self,w1,w2):
		lens=len(w1)
		cosine_sim=[]
		for i in range(lens):
			sims=self.similar_string(w1[i],w2[i])
...
def result():
	...
	tfidf=TFIDF()
	w1=tfidf.loadModel(corpus1_path)
	# print("\n"*10)
	w2=tfidf.loadModel(corpus2_path)
	tfidf.compute_tfidf(w1,w2)
